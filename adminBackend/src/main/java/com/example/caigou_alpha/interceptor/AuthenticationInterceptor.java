...
//                String userId;
                String adminUserId;
                try {
                    adminUserId = JWT.decode(token).getAudience().get(0);
                } catch (JWTDecodeException j) {
                    throw new RuntimeException("401");
                }
//                User user = userService.findUserById(userId);
                AdminUser adminUser = adminUserService.findById(Integer.parseInt(adminUserId));
                if (adminUser == null) {
                    throw new RuntimeException("用户不存在，请重新登录");
                }
                // 验证 token
                JWTVerifier jwtVerifier = JWT.require(Algorithm.HMAC256(adminUser.getApp_admin_password())).build();
                try {
